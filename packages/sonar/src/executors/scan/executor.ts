...
export default async function runExecutor(
  ...
  try {
    ...
    logger.error(
      `The Sonar scan failed for project '${context.projectName}'. Error: ${e}`
    );
    success = false;
  }
  return { success };
...
async function scan(options: ScanExecutorSchema, context: ExecutorContext) {
  ...
  if (options.verbose) {
    logger.debug(
      `Scanning project '${context.projectName}' with Sonar and opts:`,
      options
    );
  }
  ...
  if (options.autoSourcesDetection) {
    ...
    const appInfo = await getExecutedAppInfo(context, {
...
function mergeScannerOptsWithAppInfo(
  scannerOptions: Record<string, string>,
  appInfo: AppInfo
) {
...
function expandScannerOptions(
  scannerOptions: Record<string, string>,
  appInfo: AppInfo
) {
  return Object.entries(scannerOptions).reduce(
    (newScannerOpts, [optionKey, optionRawValue]) => {
      ...
      newScannerOpts[optionKey] = optionRawValues
        .map((optionValue) => {
          ...
          if (match && match[1]) {
            ...
            // Replace vars
            const values = appInfo.workspaceDependencies.map((d) =>
              matchedFilePath.replace('{projectRoot}', d.root)
            );
            return values.join(',');
          }
          return optionValue;
